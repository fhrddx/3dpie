import { AmbientLight, BufferGeometry, Color, DirectionalLight, ExtrudeGeometry, Group, Mesh, MeshPhongMaterial, OrthographicCamera, Points, Raycaster, Scene, ShaderMaterial, Shape, Sprite, SpriteMaterial, TextureLoader, Vector2, Vector3, WebGLRenderer } from "three";
...
export default class Pie {
  ...
  constructor(option: IPie) {
    ...
    this.observer = new ResizeObserver((entries) => {
      if (option.dom) {
        entries.forEach((dom) => {
          const { width, height } = dom.contentRect;
    ...
    if (option.dom) {
      this.observer.observe(option.dom);
    }
  ...
  //创建饼形图
  createPieChart(){
    const data = [{ label: '正常电站', value: 124 }, { label: '断链电站', value: 45 }, { label: '告警电站', value: 20 }];
    const colors = ['#4472c4', '#d1d4db', '#dd5847', '#dea72f', '#3b64a7', '#639746', '#96b7db', '#Eca5bc', '#d06c34', '#8f8f8f', '#dea72f', '#3b64a7', '#639746', '#96b7db', '#Eca5bc'];
  ...
  //创建一个弧形柱体
  async createSector(outRadius: number, innerRadius: number, startAngle: number, endAngle: number, depth: number, color: any, value: number, className: string) {
      ...
      vertexShader: `
        ...
        void main(){
          ...
          if(u_position.z <= 0.0){
            ...
            float p2 = z / 400.0;
            float size = 50.0 * sin((p2 + 0.1) * 3.1415926);
            gl_PointSize = size;
  ...
  public render() {
    ...
    //时间参数动起来
    this.uniforms.iTime.value += 0.15
  }
